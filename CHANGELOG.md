...
- **Pausing then immediately resuming could leave the belt stopped** — The background stats monitor only noticed a pause on its next ~1s poll cycle. An immediate resume could flip the state back to "running" before the old monitor task exited, so its status polls interleaved with the resume command sequence and confused the device. Starting, pausing, resuming, and ending a session now each cancel the monitor as part of one ordered sequence before any device command is sent, instead of racing it from a separate thread.
